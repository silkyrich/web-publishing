...
> 🤖 **Claude — how to tick these boxes live.** When the human reads you their pairing code
> (e.g. `WAVE-7321`), report each finished step by POSTing to *this site's* pairing API:
> `POST https://«this-host»/api/session/«CODE»/step` with JSON body `{"step":"L1-4","done":true}`.
> The matching box ticks itself on every open copy of the page — no refresh. Step ids match the
> labels here: `L0-1`…`L7-4` for steps, `L0-done`…`L7-done` for the green checkpoints, and
> `L0-overview`…`L7-overview` for the journey list at the top. If the POST fails (site not yet
> deployed), just say "tick box L1-4" out loud — the page falls back to remembering it locally.
